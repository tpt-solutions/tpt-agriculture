const BIP39_ENGLISH: string[] = [
  ...
  "crystal","cube","culture","cup","cupboard","curious","current","curtain","curve","custom",
  ...
  "month","moral","more","morning","mosquito","mother","motion","motor","mountain","mouse",
...
export function entropyToMnemonic(entropy: Uint8Array): string[] {
  if (entropy.length !== 32) throw new Error("Entropy must be 32 bytes (256 bits) for 24 words");
...
export function mnemonicToEntropy(words: string[]): Uint8Array {
  if (words.length !== 24) throw new Error("Mnemonic must have 24 words");

  const bits: number[] = [];
  for (const word of words) {
    const index = BIP39_ENGLISH.indexOf(word);
    if (index === -1) throw new Error(`Unknown word: "${word}"`);
    const wordBits = uintToBits(index, 11);
    bits.push(...wordBits);
  }
...
function uintToBits(value: number, width: number): number[] {
  const bits: number[] = [];
